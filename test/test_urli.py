...
def test_check():
    out = urli.categorize_urls({"https://httpstat.us/200"})
    assert len(out["OK"]) == 1

    out = urli.categorize_urls({"https://httpstat.us/404"})
    assert len(out["Client errors"]) == 1

    # "other errors"
    out = urli.categorize_urls({"https://this-doesnt-exist.doesit"})
    assert len(out["Other errors"]) == 1

    # redirect
    out = urli.categorize_urls({"https://httpstat.us/302"})
